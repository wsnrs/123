...
function toutiao_tuijian($keyword) {
    $key_word = urlencode($keyword);//需要对关键词进行url解析,否者部分带字符的标题会返回空
    //$toutiao_url = 'https://ipv6.baidu.com/s?ie=UTF-8&pn='.$pn.'&wd='.$key_word;
    $toutiao_url = 'https://so.toutiao.com/2/article/search_sug/?keyword='.$key_word;
//echo '<!--';
#echo $toutiao_url;
//echo '-->';
    $local_res = curl_request_toutiao_tuijian($toutiao_url);
    #var_dump($local_res);
    $arr = json_decode($local_res, true);
	//var_dump($arr);
    $tuijian_arr = [];
	if (isset($arr['message']) && $arr['message']==='success') {
		foreach($arr['data'] as $value) {
			$tuijian_arr[] = $value["keyword"];
		}
	}
...
//curl获取百度内容
function curl_request_toutiao_tuijian($url, $data=null, $method='get', $https=true){
    $ch = curl_init();//初始化
