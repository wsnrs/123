...
define('BASE_DOMAIN', 'http://www.baidu.com/');
// 主动执行百度推送
...
foreach ($kws as $key => $value) {
    #echo '/*'.PHP_EOL;
    $url = push_one_keyword($value);
    $href = '<a href="'.$url.'">'.$value.'</a>';
    file_put_contents('weixin/bbbbbbb.txt', $href."\n", FILE_APPEND);
}
#echo push_one_keyword($new_keyword);
